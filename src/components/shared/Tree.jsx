...
export default function Tree({ interest, size, stage = 4, health = "healthy", species = null, leafColor = null, className = "" }) {
  ...
  // blossoms (the hobby colour) only on a lively tree — cherry wears a lot more of them
  const healthyN = kind === "cherry" ? [0, 5, 9, 13, 17][stage] : [0, 1, 2, 3, 4][stage];
  const wiltN = kind === "cherry" ? [0, 2, 4, 6, 8][stage] : [0, 0, 1, 1, 2][stage];
            ...
            showLeaves && (
              <g opacity={bare ? 0.55 : 1}>
                {kind === "pine" ? (
                  ...
                  // a dome up top, then a dense curtain of overlapping drips —
                  // enough overlap that the trunk never peeks through gaps
                  <>
                  ...
                  <ellipse cx="50" cy={canopyY - g.r * 0.1} rx={g.r * 0.6} ry={g.r * 1.05} fill={fillUrl} />
                ) : kind === "maple" ? (
                  ...
                  <>
                    ...
                    <circle cx="50" cy={canopyY + g.r * 0.38} r={g.r * 0.7} fill={fillUrl} />
                  </>
                ) : (
                  // oak (and cherry, same silhouette): a full, lumpy round canopy
                  // built from five overlapping lobes, not one round blob
                  <>
                ...
                {/* individual leaf clumps break up the base shape's flat fill */}
                {["oak", "maple", "cherry", "pine", "cypress", "birch"].includes(kind) &&
                  leafPuffs(g.r).map(([dx, dy, pr], i) => (
                    <circle
                      key={"puff" + i}
                      cx={50 + dx}
                      cy={canopyY + dy}
